...
@Component({
    ...
    styleUrls: ['./candidates-add.component.scss']
})
export class CandidatesAddComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        if (this.edit_id) {
            ...
            this.jobsService.getCandidatesInfoById(+this.edit_id).subscribe(
                (res) => {
                    ...
                    if (this.applicationEdit) {
                        Object.keys(this.applicationForm.value).forEach(field => {
                            if (this.applicationEdit[field]) {
                                ...
                                {
                                    this.applicationForm.get(field).setValue(this.applicationEdit['category']['id']);
                                }
                                else {
    ...
    SaveApplication = async () => {
        ...
        if ((this.applicationForm.valid) || (this.edit_id && this.applicationForm.valid)) {
            this.spinner.show();
            ...
            this.jobsService.saveCandidatesInfo(applicationData).subscribe(
                (res) => {
                    if (res === 'success') {
                        this.flashMessageService.pop('success', 'Applicant Added Successfully', 'Applicant Add');
                        this.router.navigate(['/jobs/candidates-info']);
                    } else if (res === 'updated') {
