...
@Component({
    selector: 'app-ba-create-project',
    templateUrl: 'ba-create-project.component.html',
    styleUrls: ['./ba-create-project.component.scss']
})

export class BaCreateProjectComponent implements OnInit {
    ...
    getProjectById = () => {
        ...
        this.companyProjectService.getBaProjectById(this.project_id, !this.fromTl).subscribe(
            (res) => {
                ...
                if (this.projectData) {
                    ...
                    if (this.projectData.AssignedBAs && this.projectData.AssignedBAs.length > 0) {
                        const selected = [];
                        for (const jr of this.projectData.AssignedBAs) {
                            selected.push({
                                item_id: jr.emp_id.id,
                                item_text: jr.emp_id.firstname + '' + jr.emp_id.lastname
                            });
                        }

                        if (this.userData.id != selected[0])
    ...
    saveProject = async () => {
        if (this.projectForm.valid) {
            ...
            this.companyProjectService.saveProjectBa(postData).subscribe(
                (res) => {
                    this.spinner.hide();
